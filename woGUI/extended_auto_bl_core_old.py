...
class GlobalVars(object):
    ...
    def convert_to_schematic(self):
        ...
        for mode, key in zip(mode_lst, self.mode_data.values()):
            mode['name'] = key['name']
            mode['shell_AP'] = None if not key['shell_AP'] else BATTLE_LOG_SHELL_TYPES.ARMOR_PIERCING
            mode['shell_APCR'] = None if not key['shell_APCR'] else BATTLE_LOG_SHELL_TYPES.ARMOR_PIERCING_CR
            mode['shell_HEAT'] = None if not key['shell_HEAT'] else BATTLE_LOG_SHELL_TYPES.HOLLOW_CHARGE
            mode['shell_HE'] = (None,) if not key['shell_HE'] else (BATTLE_LOG_SHELL_TYPES.HE_MODERN, BATTLE_LOG_SHELL_TYPES.HE_LEGACY_STUN, BATTLE_LOG_SHELL_TYPES.HE_LEGACY_NO_STUN)
    ...
    def updateConfigData(self):
        self.config_data = {'mode': self.active_idx, 'ignored': self.ignored, 'friends': self.friends,
...
class SchematicForMode(object):
    ...
    def __init__(self, name='', shell_AP=None, shell_APCR=None, shell_HEAT=None,
        ...
        if tanklist is None:
            tanklist = [None]
        self.name = name
        self.shell_list = {shell_AP, shell_APCR, shell_HEAT}
        self.shell_list.update(shell_HE)
